...
* twislave.c
*
* Created: 23-Dec-21 5:35:30 PM
* Author: Falk
* Source: https://rn-wissen.de/wiki/index.php/TWI_Slave_mit_avr-gcc
*
* We support all three types of I2C transactions:
* - Pure write: One byte can be written to address 0x00, 0x01 or 0x02.
*	0x00 is the status register. 0x01 and 0x02 configure the 7 GPIO pins

* - Pure read: 254 bytes can be read, starting from address 0x00.
*    The data consists of one status byte (at 0x0), two bytes for GPIO, a CRC byte, a length byte and either DS addresses or temperatures.
